...
module.exports.generateInvoice = async (invoiceData, productDetails) => {
  return new Promise((resolve, reject) => {
    ...
    doc.text("2WHEELS", { align: "center" }).moveDown();

    // Add delivery address field
    doc.text("Delivery Address:", { underline: true }).moveDown();
    doc.fontSize(14).text(`Name: ${invoiceData?.address.full_name}`);
    doc.text(`Apartment: ${invoiceData?.address.street_name}`);
    doc.text(`Street: ${invoiceData?.address.apartment_number}`);
    ...
    doc.text(`Mobile Number: ${invoiceData?.address.mobile_Number}`).moveDown();

    // Add seller address field
    doc.text("Seller Address:", { underline: true }).moveDown();
    doc.fontSize(14).text(`Name:2Wheels ltd`);
    doc.text(`Street: Vytilla`);
    doc.text(`City: Cochin`);
    doc.text(`State: Kerala`);
    doc.text(`Postal Code: 682032`);
    doc.text(`Mobile Number: 7012066805`).moveDown();
